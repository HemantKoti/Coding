...
vector<Interval> insert(vector<Interval> &intervals, Interval newInterval) {
	...
	if (newInterval.end < intervals[0].start
			|| newInterval.start > intervals[size - 1].end) {
		if (newInterval.end < intervals[0].start) {
			result.push_back(newInterval);
		}
	...
	for (int i = 0; i < intervals.size(); i++) {
		...
		if (!intersect) {
			...
			if (i < size - 1 && newInterval.start > intervals[i].end
		...
		while (i < size && intersect) {
			...
			intersect =
					(i == size) ?
							false : doesIntersect(intervals[i], newInterval);
		}
		i--;

		// Push the merged interval to result
		Interval merged(min(newInterval.start, intervals[si].start),
